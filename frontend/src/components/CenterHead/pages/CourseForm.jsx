...
const CourseFormNew = ({ viewMode = 'center-head' }) => {
  ...
  // Track if sessions have been auto-generated to prevent duplicates
  const [sessionsAutoGenerated, setSessionsAutoGenerated] = useState(false);

  // Track if user has been warned about reducing session count
  const [showReduceWarning, setShowReduceWarning] = useState(false);
  const [pendingSessionCount, setPendingSessionCount] = useState(null);
  ...
  const tabs = [
    ...
    { id: "clo", label: "CLO", icon: "ph ph-target" },
    ...
    if (isEdit && !loadingProgram && courseId) {
      const fetchCourseData = async () => {
        try {
          ...
          // Allow editing all courses regardless of status
  ...
  }, [isEdit, courseId, programId, loadingProgram, navigate, basePath]);

  // Auto-generate missing sessions based on local data only (one time only)
  useEffect(() => {
    if (activeTab === "sessions" && !loading && formData.numberOfSessions > 0 && !sessionsAutoGenerated) {
      const currentSessionCount = formData.sessions.length;
      const requiredSessionCount = formData.numberOfSessions;

      // If we have fewer sessions than required, auto-generate the missing ones
      if (currentSessionCount < requiredSessionCount) {
        const missingCount = requiredSessionCount - currentSessionCount;

        // Auto-generate missing sessions locally (only in formData)
        const newSessions = [];
        for (let i = currentSessionCount + 1; i <= requiredSessionCount; i++) {
          newSessions.push({
            title: `Buổi ${i}`,
            order: i,
            content: '',
        ...
        // Update formData with new sessions
        setFormData(prev => ({
          ...prev,
        ...
        setSessionsAutoGenerated(true);

        console.log(`Auto-generated ${missingCount} missing sessions locally`);
      }
    }
  }, [activeTab, formData.sessions.length, formData.numberOfSessions, loading, sessionsAutoGenerated]);

  const handleInputChange = (e) => {
    const { name, value } = e.target;

    // Special handling for numberOfSessions when reducing count
    if (name === 'numberOfSessions') {
      const newCount = parseInt(value) || 0;
      const currentSessionCount = formData.sessions.length;

      // If user is reducing the count below current sessions, show warning
      if (newCount < currentSessionCount && newCount > 0) {
        setPendingSessionCount(newCount);
        setShowReduceWarning(true);
        return; // Don't update formData yet
      }

      // Reset auto-generation flag when numberOfSessions changes
      setSessionsAutoGenerated(false);
    }
  ...
  const handleClearSessionContent = (index) => {
    Swal.fire({
      title: "Xác nhận xóa nội dung",
      text: `Bạn có chắc chắn muốn xóa nội dung của buổi học "${formData.sessions[index]?.title}" không?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      cancelButtonColor: "#3085d6",
      confirmButtonText: "Xóa nội dung",
      cancelButtonText: "Hủy"
    }).then((result) => {
      if (result.isConfirmed) {
        setFormData((prev) => ({
          ...
          sessions: prev.sessions.map((session, i) =>
            i === index
              ? { ...session, content: '', learningType: 'theory' }
              : session
          ),
        }));
        toast.success('Đã xóa nội dung buổi học!', { position: "top-right" });
      }
    });
  };

  const handleConfirmReduceSessions = () => {
    if (pendingSessionCount !== null) {
      // Remove excess sessions
      setFormData((prev) => ({
        ...prev,
        sessions: prev.sessions.slice(0, pendingSessionCount),
        numberOfSessions: pendingSessionCount
      }));

      setSessionsAutoGenerated(false);
      setShowReduceWarning(false);
      setPendingSessionCount(null);
      toast.success(`Đã giảm số lượng buổi học xuống ${pendingSessionCount}!`, { position: "top-right" });
    }
  };

  const handleCancelReduceSessions = () => {
    setShowReduceWarning(false);
    setPendingSessionCount(null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate back to course detail page instead of program edit
      navigate(`${basePath}/programs/${programId}/courses/${courseId}/details`);
    } catch (error) {
                              ...
                              <td className="text-center">
                                <div className="d-flex gap-1 justify-content-center">
                                  ...
                                  <Button
                                    ...
                                    icon="ph ph-eraser"
                                    size="sm"
                                    onClick={() => handleClearSessionContent(index)}
                                    title="Xóa nội dung"
                                  />
                                </div>
                              </td>
            ...
            <div className="modal-content">
              <div className="modal-header bg-warning-50">
                <h5 className="modal-title">
                  ...
                  Cảnh báo giảm số lượng buổi học
                </h5>
              </div>
              <div className="modal-body">
                <div className="alert alert-warning">
                  <p className="mb-2">
                    Bạn đang giảm số lượng buổi học từ <strong>{formData.numberOfSessions}</strong> xuống <strong>{pendingSessionCount}</strong>.
                  </p>
                  <p className="mb-2">
                    Hiện tại có <strong>{formData.sessions.length} buổi học</strong> đã được tạo.
                  </p>
                  <p className="mb-0">
                    Nếu tiếp tục, <strong>{formData.sessions.length - pendingSessionCount} buổi học cuối cùng sẽ bị xóa</strong>.
                  </p>
                </div>

                <div className="text-center">
                  <p className="text-muted mb-0">Bạn có muốn tiếp tục không?</p>
                </div>
              </div>
              <div className="modal-footer">
                <Button
                  variant="outline"
                  onClick={handleCancelReduceSessions}
                  disabled={loading}
                >
                  Hủy bỏ
                </Button>
                <Button
                  variant="danger"
                  onClick={handleConfirmReduceSessions}
                  disabled={loading}
                  icon="ph ph-trash"
                >
                  Xóa buổi học thừa
