...
const ClassScheduleInner = () => {
    ...
    const fetchSchedules = async () => {
      try {
        ...
        if (classId) {
          ...
          // Lấy tên lớp từ response nếu có
  ...
  const getStatusColor = (date, startTime, endTime) => {
    ...
    const classStartTime = new Date(`${date}T${startTime}`);
    const classEndTime = new Date(`${date}T${endTime}`);
    
    if (now < classStartTime) {
      return 'text-info'; // Chưa đến giờ
    } else if (now >= classStartTime && now <= classEndTime) {
      return 'text-success'; // Đang diễn ra
    } else {
      return 'text-secondary'; // Đã kết thúc
    }
  };
        ...
        <div className='flex-between gap-16 flex-wrap mb-40'>
          <div>
            ...
            <span className='text-neutral-500'>
              Hiển thị {schedules.length} buổi học
            </span>
          </div>
                  ...
                  <div className='p-20 position-relative w-lg-50'>
                    ...
                    <div className='mb-20'>
                      <div className='flex-align gap-8 mb-8'>
                        ...
                        <span className='text-neutral-500 text-lg'>
                          {schedule.room?.room_name || 'Chưa xác định phòng'}
                        </span>
                      </div>
                      {schedule.room?.location && (
                        <div className='text-neutral-400 text-sm ms-8'>
                          {schedule.room.location}
                        </div>
                      )}
                    </div>
                    ...
                    <div className='flex-between gap-8 pt-24 border-top border-neutral-50 mt-28 border-dashed border-0'>
                      <button
                        onClick={() => handleViewAttendance(schedule._id)}
                        className='flex-align gap-8 text-main-600 hover-text-decoration-underline transition-1 fw-semibold btn btn-link p-0'
                      >
                        Xem điểm danh
...
export default ClassScheduleInner;
