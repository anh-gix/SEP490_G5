...
const Step5SessionGeneration = ({ currentCourse, onNext, onPrevious }) => {
  ...
  const fetchCourseSessions = async () => {
    try {
      const response = await courseService.getCourseById(currentCourse._id);
  ...
  const handleGenerateSessions = async () => {
    const numberOfSessions = parseInt(prompt('Nhập số lượng buổi học:', currentCourse.numberOfSessions || 30));
    if (!numberOfSessions || numberOfSessions <= 0) return;
    ...
    try {
      ...
      await courseService.updateCourse(currentCourse._id, {
  ...
  const handleDeleteSession = async (sessionId) => {
    if (!window.confirm('Bạn có chắc muốn xóa buổi học này?')) return;

    try {
      await sessionService.deleteSession(sessionId);
      await courseService.updateCourse(currentCourse._id, {
        sessions: sessions.filter(s => s._id !== sessionId).map(s => s._id)
      });
      await fetchCourseSessions();
  ...
  if (!currentCourse) {
    return (
      <div className="text-center py-32">
        <p className="text-neutral-600">Vui lòng chọn một học phần từ bước trước.</p>
        <Button variant="primary" onClick={onPrevious}>Quay lại chọn học phần</Button>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="alert alert-info mb-24">
        ...
        Đang tạo sessions cho học phần: <strong>{currentCourse.name}</strong>
      </div>
                ...
                {sessions.map((session, index) => (
                  <tr key={session._id}>
                    <td className="px-16 py-12">
                      <Badge variant="secondary">{session.order || index + 1}</Badge>
                    </td>
                    ...
                    <td className="px-16 py-12">
                      {editingSession === session._id ? (
                        ...
                        (session.clos || []).map(cloId => {
                          ...
                          return clo ? (
                            <Badge key={clo._id} variant="success" className="me-1">{clo.code}</Badge>
                          ) : null;
                        })
                      )}
                    </td>
      ...
      <div className="d-flex justify-content-between gap-3">
        ...
        <Button variant="primary" onClick={onNext} icon="ph ph-arrow-right" iconPosition="right">
          Lưu & Tiếp tục
        </Button>
      </div>
    </div>
...
export default Step5SessionGeneration;
