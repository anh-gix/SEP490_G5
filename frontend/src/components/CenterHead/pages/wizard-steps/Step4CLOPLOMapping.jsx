...
const Step4CLOPLOMapping = ({ programData, currentCourse, setCurrentCourse, onNext, onPrevious }) => {
  ...
  const getPLODetails = (ploId) => {
    if (!programData.plos) return null;
    return programData.plos.find(p => (p._id || p) === ploId);
  };
  ...
  return (
    <div>
      <div className="alert alert-info mb-24">
        ...
        Đang tạo CLO cho học phần: <strong>{currentCourse.name}</strong>
      </div>
                ...
                {clos.map((clo, index) => (
                  <tr key={clo._id}>
                    ...
                    <td className="px-16 py-12">
                      <Badge variant="success">{clo.code}</Badge>
                    </td>
                    ...
                    <td className="px-16 py-12">
                      {(clo.mappedPLOs || []).map(ploId => {
                        ...
                        return plo ? (
                          <Badge key={ploId} variant="primary" className="me-1">
                            {plo.code || ploId}
                          </Badge>
                        ) : null;
                      })}
                    ...
                    <td className="px-16 py-12 text-center">
                      <Button
                        variant="danger"
                        size="sm"
                        onClick={() => handleDeleteCLO(clo._id)}
                        icon="ph ph-trash"
                      />
                    </td>
                  </tr>
...
export default Step4CLOPLOMapping;
