  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`badge ${config.class} text-white`}>
        <i className={`ph ${config.icon} me-1`}></i>
        {config.text}
      </span>
    );
  };
      ...
      <div className='blog-page-section py-120'>
        <div className='container'>
          <div className='text-center'>
            ...
            <button 
              className='btn btn-main-600 mt-3'
              onClick={() => navigate(-1)}
            >
              Quay lại
            </button>
      ...
      <div className='container'>
        ...
        <div className='flex-between gap-16 flex-wrap mb-40'>
          ...
          <div className='flex-align gap-16'>
            ...
            <div className='text-neutral-500'>
              Tổng: {attendanceList.length} sinh viên
            </div>
          </div>
        </div>
          ...
          <div className='col-md-3'>
            <div className='card bg-success text-white'>
              <div className='card-body text-center'>
                ...
                <h4>{attendanceList.filter(item => item.attendance?.status === 'present').length}</h4>
                <p className='mb-0'>Có mặt</p>
              </div>
            </div>
          </div>
          <div className='col-md-3'>
            <div className='card bg-danger text-white'>
              <div className='card-body text-center'>
                <i className='ph ph-x-circle text-4xl mb-2'></i>
                <h4>{attendanceList.filter(item => item.attendance?.status === 'absent').length}</h4>
                <p className='mb-0'>Vắng mặt</p>
              </div>
            </div>
          </div>
          <div className='col-md-3'>
            <div className='card bg-warning text-white'>
              <div className='card-body text-center'>
                <i className='ph ph-clock text-4xl mb-2'></i>
                <h4>{attendanceList.filter(item => item.attendance?.status === 'late').length}</h4>
                <p className='mb-0'>Đi muộn</p>
              </div>
            </div>
          </div>
          <div className='col-md-3'>
            <div className='card bg-info text-white'>
              <div className='card-body text-center'>
                <i className='ph ph-file-text text-4xl mb-2'></i>
                <h4>{attendanceList.filter(item => item.attendance?.status === 'excused').length}</h4>
                <p className='mb-0'>Có phép</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {attendanceList.length === 0 ? (
          <div className='text-center py-5'>
            <div className='mb-4'>
              <i className='ph ph-users text-neutral-300' style={{ fontSize: '4rem' }}></i>
            </div>
                        ...
                        <tr key={item._id}>
                          ...
                          <td className='py-16 px-20'>
                            <div className='d-flex align-items-center'>
                              <div className='avatar-sm bg-main-100 text-main-600 rounded-circle d-flex align-items-center justify-content-center me-3'>
                          ...
                          <td className='py-16 px-20'>
                            {getStatusBadge(item.attendance?.status || 'absent')}
                          </td>
                          <td className='py-16 px-20'>
                            {item.attendance?.markedAt ? (
                              <div>
                                <div className='text-sm'>
                                  {new Date(item.attendance.markedAt).toLocaleString('vi-VN')}
                                </div>
                                {item.attendance.markedBy && (
                                  <small className='text-neutral-500'>Bởi giáo viên</small>
                                )}
                              </div>
                            ) : (
                              <span className='text-neutral-400'>Chưa điểm danh</span>
                            )}
                          </td>
                          <td className='py-16 px-20'>
                            {canMark ? (
                              <div className='btn-group' role='group'>
                                <button
                                  className={`btn btn-sm ${item.attendance?.status === 'present' ? 'btn-success' : 'btn-outline-success'}`}
                                  onClick={() => handleMarkAttendance(item._id, 'present')}
                                  disabled={isUpdating}
                                >
                                  {isUpdating ? (
                                ...
                                <button
                                  className={`btn btn-sm ${item.attendance?.status === 'absent' ? 'btn-danger' : 'btn-outline-danger'}`}
                                  onClick={() => handleMarkAttendance(item._id, 'absent')}
                                  disabled={isUpdating}
                                >
                                  {isUpdating ? (
                                    <span className='spinner-border spinner-border-sm me-1'></span>
                                ...
                                <button
                                  className={`btn btn-sm ${item.attendance?.status === 'late' ? 'btn-warning' : 'btn-outline-warning'}`}
                                  onClick={() => handleMarkAttendance(item._id, 'late')}
                                  disabled={isUpdating}
                                >
                                  {isUpdating ? (
                                    <span className='spinner-border spinner-border-sm me-1'></span>
                                ...
                                <button
                                  className={`btn btn-sm ${item.attendance?.status === 'excused' ? 'btn-info' : 'btn-outline-info'}`}
                                  onClick={() => handleMarkAttendance(item._id, 'excused')}
                                  disabled={isUpdating}
                                >
                                  {isUpdating ? (
                                    <span className='spinner-border spinner-border-sm me-1'></span>
                                  ) : (
                                    <i className='ph ph-file-text me-1'></i>
                                  )}
                                  Phép
                                </button>
                              </div>
                              ...
                              <span className='text-neutral-400 text-sm'>
                                {scheduleInfo ? 'Không thể điểm danh' : 'Chưa có thông tin lịch học'}
                              </span>
                            )}
                          </td>
                        </tr>
...
export default AttendanceDetailInner;
